...
app.config['EVIDENCE_FOLDER'] = os.path.join(app.config['PROCESSED_FOLDER'], 'evidence')
...
def detect_webcam():
    ...
    try:
        ...
        if frame is None:
            return jsonify({'error': 'Không thể đọc frame'}), 400
...
# === Routes mới cho trang kết quả huấn luyện (ĐÃ CẬP NHẬT) ===
